...
import 'package:trapp/src/pages/forgot.dart';
import 'package:trapp/src/pages/login.dart';
import 'package:trapp/src/pages/mobile_initiate.dart';
...
class MobileVerificationPage extends StatefulWidget {
  ...
  MobileVerificationPage({@required this.mobile, this.callback});
...
class _MobileVerificationPageState extends State<MobileVerificationPage> {
  ...
  void _authProviderListener() async {
    ...
    if (_authProvider!.authState.progressState == 4) {
      ...
      if (AuthProvider.of(context).authState.userModel!.id == null) {
        ...
        //TODO:: mark the phoneVerified as true
      }
    ...
    return WillPopScope(
      onWillPop: () async {
        if (AuthProvider.of(context).authState.userModel!.id == null) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (BuildContext context) => MobileInitiatePage(callback: widget.callback),
            ),
          );
          return false;
        }
  ...
  _onVerifyPressed() async {
    ...
    await _keicyProgressDialog!.show();
    _authProvider!.verifyMobileVerification(
