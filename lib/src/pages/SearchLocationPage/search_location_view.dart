...
class _SearchLocationViewState extends State<SearchLocationView> with WidgetsBindingObserver {
  ...
  void _appDataProviderListener() async {
    ...
    if (_appDataProvider!.appDataState.progressState == 2 && _appDataProvider!.appDataState.categoryList!.isNotEmpty) {
      if (widget.isPopup) {
    ...
    if (_appDataProvider!.appDataState.progressState == 2 && _appDataProvider!.appDataState.categoryList!.isEmpty) {
      ErrorDialog.show(
        context,
        widthDp: widthDp,
        heightDp: heightDp,
    ...
    return Container(
      ...
      child: GestureDetector(
        onTap: () async {
          ...
          Position? position = await AppDataProvider.of(context).getCurrentPosition();
