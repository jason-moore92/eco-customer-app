...
class AuthProvider extends ChangeNotifier {
  ...
  void resendVerifyLink({@required String? email, @required String? password}) async {
    try {
      var result = await UserApiProvider.resendVerifyLink(email!, password!);
      if (result["success"]) {
        _authState = _authState.update(
          progressState: 2,
          message: "",
  ...
  void forgotPassword({@required String? email}) async {
    try {
      var result = await UserApiProvider.forgotPassword(email: email);
      if (result["success"]) {
        _authState = _authState.update(
          progressState: 3,
          message: "",
  ...
  void verifyOTP({
    ...
    try {
      var result = await UserApiProvider.verifyOTP(
        otp: otp,
        email: email,
  ...
  void verifyMobileVerification({
    @required int? token,
    @required String? mobile,
  }) async {
    try {
      var result = await UserApiProvider.verifyMobileVerification(mobile: mobile, token: token);
      if (result["success"]) {
        _authState = _authState.update(
          progressState: 4,
          message: "",
