...
//   static Stream<int>? getChatRoomsLengthStream({
//     @required String? chatRoomType,
...
//       query = ref;
//       query = ref;
//       // if (searchKey!.isNotEmpty) query = query.orderBy("userName");
//       query = query.orderBy("updateAt", descending: true);
//       // if (searchKey.isNotEmpty) query = query.startAt([searchKey]).endAt(["$searchKey\uf8ff"]);
//       query = query.where("ids", arrayContains: "Business-$storeId");
//       if (limit != null) query = query.limit(limit);
//       return query.snapshots().map((snapshot) {
...
//       return null;
//     }
//     // return KeicyFireStoreDataProvider.instance.getDocumentsLengthStream(path: path, wheres: wheres, orderby: orderby, limit: limit);
//   }
}
